...
`reader.go:47` documents "All exported methods are safe for concurrent use". They are not: `r.recOff`, `r.chunkIdx`, `r.freeOff` and the shared `r.buf` are unguarded, and `go test -race` passes only because no test reads concurrently. cee-exporter does not use the `Reader`, so this is about the doc and the code agreeing.
...
- Produces: a tagged `v0.6.0` that cee-exporter can bump to.
...
- [ ] `v0.6.0` tagged and pushed; cee-exporter can `go get github.com/fjacquet/go-evtx@v0.6.0`.
