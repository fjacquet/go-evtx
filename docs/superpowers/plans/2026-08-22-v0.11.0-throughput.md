...
**Goal:** Let a caller opt into group-commit fsync and batch writes, and cut per-record allocation, so a CEPA burst is no longer bound by one fsync per 64 KiB chunk.
...
// BenchmarkWriteRecordsBatch measures the batch entry point at a batch size a
// CEE receiver would plausibly use.
