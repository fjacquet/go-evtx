...
- **`OnFsync` fires on every sync** — from `WriteRecord`, `WriteRecords`, `WriteRaw`, `rotate`, `Close` and the background flush tick, not only when `FlushIntervalSec > 0`. It is invoked after `w.mu` is released, so a callback may safely call most `Writer` methods — except `Close`: a callback fired from the background goroutine's own fsync drain that calls `Close` deadlocks, because `Close` waits for that same goroutine to exit while it is blocked inside the callback. A callback that itself triggers a further flush recurses on its own call stack.
