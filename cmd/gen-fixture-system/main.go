// Command gen-fixture-system writes the same fixture as gen-fixture, plus the
// three <System> fields a real Windows event always carries: ProviderName,
// Computer and Channel.
//
// Why a second generator rather than an edit. cmd/gen-fixture is frozen —
// every row of docs/format-baseline.md compares against its exact output, and
// a repository hook enforces the freeze. This one exists because F18 changed
// what "supply nothing" means: an absent value is now encoded as NULL, and per
// libyal a NULL-valued substitution makes its element "ignored and not
// created". gen-fixture supplies none of those three, so its events lose
// <Provider Name>, <Computer> and <Channel> entirely. That is a correct
// encoding of "nothing was supplied" — the file is valid and
// EventLogReader reads every record — but Get-WinEvent's formatting layer
// throws a NullReferenceException on an event with no provider name. Real
// Windows never writes one.
//
// So the CI gate moves here, and gen-fixture stays exactly as it was, still
// reproducing every historical measurement.
package main
...
func acceptsObjectName(n int) bool {
	dir, err := os.MkdirTemp("", "gen-fixture-system-probe")
	if err != nil {
	...
	// The probe must carry the SAME fields the fixture writes, including the
	// three System ones. Probing with a smaller record finds a ceiling the
	// real write then exceeds — which is exactly what happened first time
	// here: the search returned a length valid for gen-fixture's field set,
	// the near-maximum write blew past maxRecordPayload, and the generator
	// died after 387 of its 403 records.
	fields := map[string]string{
		"ProviderName":    "Microsoft-Windows-Security-Auditing",
		"Computer":        "TESTHOST",
		"Channel":         "Security",
...
func fatal(err error) {
	fmt.Fprintln(os.Stderr, "gen-fixture-system:", err)
	os.Exit(1)
}
